...
func (t *WorkspaceRoute) putWorkspaceSimulation(c *gin.Context) {
	...
	if err != nil {
		logger.WithTraceID(c.Request.Context(), t.Logger).
			Error("failed to marshal simulation", zap.Error(err))
	...
	if err != nil {
		logger.WithTraceID(c.Request.Context(), t.Logger).
			Error("failed to update simulation", zap.Error(err))

		c.JSON(http.StatusInternalServerError, gin.H{})
		return
	}

	logger.WithTraceID(c.Request.Context(), t.Logger).Info("successfully update workspace simulation",
